/*
================================================================================
PHASE 3 FULL DATABASE SETUP - RUN THIS ON master
================================================================================
This is the complete SQL Server setup for the final pre-data-engineering phase.
It creates/repairs:
- 3 charity operational source databases
- unified_charity_platform_clean operational DB
- charity_dwh star schema for Power BI
- Phase 3 views/procedures/tables for frontend + backend APIs
================================================================================
*/
...
/* -----------------------------------------------------------------------------
   FULL RESET FOR DEMO/PRESENTATION
   This file prepares a clean environment before the Data Engineering phase.
   It drops only the demo databases used by this project, then recreates them.
----------------------------------------------------------------------------- */
IF DB_ID(N'charity_dwh') IS NOT NULL
BEGIN
    ALTER DATABASE charity_dwh SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE charity_dwh;
END;
GO

IF DB_ID(N'charity_food_bank_operational') IS NOT NULL
BEGIN
    ALTER DATABASE charity_food_bank_operational SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE charity_food_bank_operational;
END;
GO

IF DB_ID(N'charity_resala_operational') IS NOT NULL
BEGIN
    ALTER DATABASE charity_resala_operational SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE charity_resala_operational;
END;
GO

IF DB_ID(N'charity_haya_karima_operational') IS NOT NULL
BEGIN
    ALTER DATABASE charity_haya_karima_operational SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE charity_haya_karima_operational;
END;
GO

IF DB_ID(N'unified_charity_platform_clean') IS NOT NULL
BEGIN
    ALTER DATABASE unified_charity_platform_clean SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE unified_charity_platform_clean;
END;
GO

IF SUSER_ID(N'charity_app_user') IS NOT NULL
BEGIN
    DROP LOGIN charity_app_user;
END;
GO
...
GO
/* ============================================================
   ALL-IN-ONE SQL FILE â€” NO ERROR VERSION
...
/* ============================================================
   CLEAN DATABASE V1 â€” Unified Charity Platform
   Ù‡Ø¯Ù Ø§Ù„ØªØµÙ…ÙŠÙ…:
   - ERD Ù†Ø¸ÙŠÙ ÙˆÙˆØ§Ø¶Ø­
   - Relationships Ø­Ù‚ÙŠÙ‚ÙŠØ© Ø¨Ù€ Foreign Keys
   - Beneficiary 360 Ù‚ÙˆÙŠ
   - ÙƒØ´Ù duplicate / fraud Ø¨Ø¯ÙˆÙ† Ù…Ø§ Ù†ÙƒØ³Ø± Ø§Ù„Ù€ operational clean data
   - Ø¬Ø§Ù‡Ø² Ù„Ù„Ù€ Dashboard + Kafka/Outbox + Data Warehouse
   ============================================================ */
...
/* ============================================================
   ...
   Ø¨Ù…Ø§ Ø¥Ù†Ù‡Ø§ DB Ø¬Ø¯ÙŠØ¯Ø©ØŒ Ø§Ù„Ø³ÙƒØ±ÙŠØ¨Øª Ø¨ÙŠØ­Ø°Ù Ø§Ù„Ø¬Ø¯Ø§ÙˆÙ„ Ù„Ùˆ Ù…ÙˆØ¬ÙˆØ¯Ø© Ø¹Ø´Ø§Ù† ÙŠØ¨Ø¯Ø£ Ù†Ø¶ÙŠÙ.
   ============================================================ */
...
/*
   Ø¬Ø¯ÙˆÙ„ raw identity records:
   Ù‡Ù†Ø§ Ø¨Ù†Ø³Ù…Ø­ Ø¨Ø§Ù„ØªÙƒØ±Ø§Ø± Ø¹Ù…Ø¯Ø§Ù‹ Ø¹Ø´Ø§Ù† Ù†ÙƒØ´Ù duplicate/fraud.
   Ø¯Ù‡ ÙŠÙ…Ø«Ù„ Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„Ø®Ø§Ù… Ø§Ù„Ù„ÙŠ Ø¬Ø§ÙŠØ© Ù…Ù† Ø§Ù„Ø¬Ù…Ø¹ÙŠØ§Øª Ø£Ùˆ Ø§Ù„ÙÙˆØ±Ù… Ø£Ùˆ source systems.
*/
...
/* Stock balance trigger:
   ÙŠØ­Ø³Ø¨ Ø§Ù„Ø±ØµÙŠØ¯ ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹ Ù…Ù† Ø­Ø±ÙƒØ§Øª Ø§Ù„Ù…Ø®Ø²ÙˆÙ†.
*/
...
PRINT N'âœ… Clean schema created successfully in unified_charity_platform_clean.';
GO
...
/* ============================================================
   CLEAN DATABASE V1 â€” Reference + Demo Seed Data
   ============================================================ */
...
VALUES
('GOV_ADMIN', N'Ø§Ø¯Ù…Ù† Ø­ÙƒÙˆÙ…Ø©', 'Government Admin'),
('CHARITY_ADMIN', N'Ø§Ø¯Ù…Ù† Ø¬Ù…Ø¹ÙŠØ©', 'Charity Admin'),
('BENEFICIARY', N'Ù…Ø³ØªÙÙŠØ¯', 'Beneficiary'),
('DONOR', N'Ù…ØªØ¨Ø±Ø¹', 'Donor'),
('DATA_ENGINEER', N'Ù…Ù‡Ù†Ø¯Ø³ Ø¨ÙŠØ§Ù†Ø§Øª', 'Data Engineer');
...
VALUES
('GOV-CAI', N'Ø§Ù„Ù‚Ø§Ù‡Ø±Ø©', 'Cairo'),
('GOV-GIZ', N'Ø§Ù„Ø¬ÙŠØ²Ø©', 'Giza'),
('GOV-MNF', N'Ø§Ù„Ù…Ù†ÙˆÙÙŠØ©', 'Menofia');
...
VALUES
(@cairo, 'CITY-CAI-01', N'Ù…Ø¯ÙŠÙ†Ø© Ù†ØµØ±', 'Nasr City'),
(@cairo, 'CITY-CAI-02', N'Ø§Ù„Ù…Ø¹Ø§Ø¯ÙŠ', 'Maadi'),
(@giza,  'CITY-GIZ-01', N'Ø§Ù„Ø¯Ù‚ÙŠ', 'Dokki'),
(@giza,  'CITY-GIZ-02', N'6 Ø£ÙƒØªÙˆØ¨Ø±', '6th of October'),
(@mnf,   'CITY-MNF-01', N'Ø´Ø¨ÙŠÙ† Ø§Ù„ÙƒÙˆÙ…', 'Shebin El Kom'),
(@mnf,   'CITY-MNF-02', N'Ù…Ù†ÙˆÙ', 'Menouf');
...
VALUES
('ORG-FOOD', N'Ø¨Ù†Ùƒ Ø§Ù„Ø·Ø¹Ø§Ù… Ø§Ù„Ù…ØµØ±ÙŠ', 'Egyptian Food Bank', '01010000001', 'food@test.com', N'Ø§Ù„Ù‚Ø§Ù‡Ø±Ø©'),
('ORG-RESALA', N'Ø¬Ù…Ø¹ÙŠØ© Ø±Ø³Ø§Ù„Ø©', 'Resala Charity', '01010000002', 'resala@test.com', N'Ø§Ù„Ø¬ÙŠØ²Ø©'),
('ORG-HAYA', N'Ø­ÙŠØ§Ø© ÙƒØ±ÙŠÙ…Ø©', 'Haya Karima', '01010000003', 'haya@test.com', N'Ø§Ù„Ù…Ù†ÙˆÙÙŠØ©');
...
VALUES
('BR-FOOD-CAI', @orgFood, N'ÙØ±Ø¹ Ø¨Ù†Ùƒ Ø§Ù„Ø·Ø¹Ø§Ù… - Ø§Ù„Ù‚Ø§Ù‡Ø±Ø©', @cairo, @nasr, N'Ù…Ø¯ÙŠÙ†Ø© Ù†ØµØ±', '01020000001'),
('BR-RESALA-GIZ', @orgResala, N'ÙØ±Ø¹ Ø±Ø³Ø§Ù„Ø© - Ø§Ù„Ø¬ÙŠØ²Ø©', @giza, @dokki, N'Ø§Ù„Ø¯Ù‚ÙŠ', '01020000002'),
('BR-HAYA-MNF', @orgHaya, N'ÙØ±Ø¹ Ø­ÙŠØ§Ø© ÙƒØ±ÙŠÙ…Ø© - Ø§Ù„Ù…Ù†ÙˆÙÙŠØ©', @mnf, @shebin, N'Ø´Ø¨ÙŠÙ† Ø§Ù„ÙƒÙˆÙ…', '01020000003');
...
VALUES
('SUP-FOOD', N'Ø¯Ø¹Ù… ØºØ°Ø§Ø¦ÙŠ', 'Food Support', 0, 1),
('SUP-MED', N'Ø¯Ø¹Ù… Ø·Ø¨ÙŠ', 'Medical Support', 1, 0),
('SUP-CASH', N'Ø¯Ø¹Ù… Ù†Ù‚Ø¯ÙŠ', 'Cash Support', 1, 0),
('SUP-CLOTH', N'Ø¨Ø·Ø§Ø·ÙŠÙ† ÙˆÙ…Ù„Ø§Ø¨Ø³', 'Clothes and Blankets', 0, 1);

INSERT INTO dbo.payment_methods (payment_method_code, method_name_ar, method_name_en)
VALUES
('PM-CASH', N'ÙƒØ§Ø´', 'Cash'),
('PM-CARD', N'Ø¨Ø·Ø§Ù‚Ø© Ø¨Ù†ÙƒÙŠØ©', 'Card'),
('PM-WALLET', N'Ù…Ø­ÙØ¸Ø© Ø¥Ù„ÙƒØªØ±ÙˆÙ†ÙŠØ©', 'Mobile Wallet');

INSERT INTO dbo.document_types (document_type_code, document_type_name_ar, document_type_name_en, is_required_by_default)
VALUES
('NATIONAL_ID', N'ØµÙˆØ±Ø© Ø§Ù„Ø¨Ø·Ø§Ù‚Ø©', 'National ID', 1),
('INCOME_PROOF', N'Ø¥Ø«Ø¨Ø§Øª Ø¯Ø®Ù„', 'Income Proof', 0),
('MEDICAL_REPORT', N'ØªÙ‚Ø±ÙŠØ± Ø·Ø¨ÙŠ', 'Medical Report', 0);

INSERT INTO dbo.inventory_items (item_code, item_name_ar, item_name_en, item_category, unit, default_unit_cost)
VALUES
('ITEM-FOOD-BOX', N'ÙƒØ±ØªÙˆÙ†Ø© ØºØ°Ø§Ø¦ÙŠØ©', 'Food Box', N'ØºØ°Ø§Ø¡', N'ÙƒØ±ØªÙˆÙ†Ø©', 450),
('ITEM-BLANKET', N'Ø¨Ø·Ø§Ù†ÙŠØ©', 'Blanket', N'Ø¨Ø·Ø§Ø·ÙŠÙ† ÙˆÙ…Ù„Ø§Ø¨Ø³', N'Ù‚Ø·Ø¹Ø©', 300),
('ITEM-MEDICINE', N'Ø£Ø¯ÙˆÙŠØ© Ø´Ù‡Ø±ÙŠØ©', 'Monthly Medicine', N'Ø·Ø¨ÙŠ', N'Ø¹Ø¨ÙˆØ©', 250);
...
VALUES
('USR-GOV-001', @roleGov, NULL, NULL, N'Ø§Ø¯Ù…Ù† Ø¹Ø§Ù…', '01030000001', 'gov@test.com', 'demo_hash'),
('USR-FOOD-ADMIN', @roleCharity, @orgFood, @brFood, N'Ø§Ø¯Ù…Ù† Ø¨Ù†Ùƒ Ø§Ù„Ø·Ø¹Ø§Ù…', '01030000002', 'food.admin@test.com', 'demo_hash'),
('USR-RESALA-ADMIN', @roleCharity, @orgResala, @brResala, N'Ø§Ø¯Ù…Ù† Ø±Ø³Ø§Ù„Ø©', '01030000003', 'resala.admin@test.com', 'demo_hash');
...
VALUES
('USR-BEN-AHMED', @roleBeneficiary, N'Ø£Ø­Ù…Ø¯ Ù…Ø­Ù…Ø¯ Ø¹Ù„ÙŠ', '01099000001', 'ahmed@test.com', 'demo_hash'),
('USR-BEN-SARA', @roleBeneficiary, N'Ø³Ø§Ø±Ø© Ù…Ø­Ù…ÙˆØ¯ Ø­Ø³Ù†', '01099000002', 'sara@test.com', 'demo_hash'),
('USR-BEN-MONA', @roleBeneficiary, N'Ù…Ù†Ù‰ Ø®Ø§Ù„Ø¯ Ø¥Ø¨Ø±Ø§Ù‡ÙŠÙ…', '01099000003', 'mona@test.com', 'demo_hash'),
('USR-BEN-YOUSSEF', @roleBeneficiary, N'ÙŠÙˆØ³Ù Ø£Ø­Ù…Ø¯ Ø³Ø§Ù„Ù…', '01099000004', 'youssef@test.com', 'demo_hash');
...
VALUES
('BEN-0001', @uAhmed, '2990101123456', N'Ø£Ø­Ù…Ø¯ Ù…Ø­Ù…Ø¯ Ø¹Ù„ÙŠ', 'Male', '1999-01-01', '01099000001', 'ahmed@test.com', @cairo, @nasr, N'Ù…Ø¯ÙŠÙ†Ø© Ù†ØµØ±', 5, 1800, N'Ø¹Ù…Ù„ ØºÙŠØ± Ù…Ù†ØªØ¸Ù…'),
('BEN-0002', @uSara, '2980202123456', N'Ø³Ø§Ø±Ø© Ù…Ø­Ù…ÙˆØ¯ Ø­Ø³Ù†', 'Female', '1998-02-02', '01099000002', 'sara@test.com', @giza, @dokki, N'Ø§Ù„Ø¯Ù‚ÙŠ', 4, 1200, N'Ù„Ø§ ØªØ¹Ù…Ù„'),
('BEN-0003', @uMona, '3010303123456', N'Ù…Ù†Ù‰ Ø®Ø§Ù„Ø¯ Ø¥Ø¨Ø±Ø§Ù‡ÙŠÙ…', 'Female', '2001-03-03', '01099000003', 'mona@test.com', @mnf, @shebin, N'Ø´Ø¨ÙŠÙ† Ø§Ù„ÙƒÙˆÙ…', 3, 1600, N'Ø·Ø§Ù„Ø¨Ø©'),
('BEN-0004', @uYoussef, '3000404123456', N'ÙŠÙˆØ³Ù Ø£Ø­Ù…Ø¯ Ø³Ø§Ù„Ù…', 'Male', '2000-04-04', '01099000004', 'youssef@test.com', @mnf, @shebin, N'Ø´Ø¨ÙŠÙ† Ø§Ù„ÙƒÙˆÙ…', 2, 1700, N'Ø·Ø§Ù„Ø¨');
...
VALUES
(@benAhmed, @orgFood, @brFood, N'PLATFORM', N'Ù…Ø³Ø¬Ù„ ÙÙŠ Ø¨Ù†Ùƒ Ø§Ù„Ø·Ø¹Ø§Ù…'),
(@benAhmed, @orgResala, @brResala, N'PLATFORM', N'Ù…Ø³Ø¬Ù„ ÙÙŠ Ø±Ø³Ø§Ù„Ø© Ø£ÙŠØ¶Ø§Ù‹'),
(@benSara, @orgResala, @brResala, N'PLATFORM', N'Ø·Ù„Ø¨ Ø¯Ø¹Ù… Ø·Ø¨ÙŠ'),
(@benMona, @orgHaya, @brHaya, N'PLATFORM', N'Ø¯Ø¹Ù… Ù…Ù„Ø§Ø¨Ø³'),
(@benYoussef, @orgFood, @brFood, N'PLATFORM', N'Ø·Ù„Ø¨ Ù…Ø±ÙÙˆØ¶');
...
VALUES
(N'food_source', @orgFood, @brFood, '2990101123456', N'Ø£Ø­Ù…Ø¯ Ù…Ø­Ù…Ø¯ Ø¹Ù„ÙŠ', '01099000001', 'ahmed@test.com', @cairo, @nasr, @benAhmed, N'MATCHED', N'{"source":"food"}'),
(N'resala_source', @orgResala, @brResala, '2990101123456', N'Ø§Ø­Ù…Ø¯ Ù… Ø¹Ù„ÙŠ', '01099000001', 'ahmed.second@test.com', @giza, @dokki, @benAhmed, N'POSSIBLE_DUPLICATE', N'{"source":"resala"}'),
(N'haya_source', @orgHaya, @brHaya, '2980202123456', N'Ø³Ø§Ø±Ø© Ù… Ø­Ø³Ù†', '01099000999', 'sara.dup@test.com', @mnf, @shebin, @benSara, N'POSSIBLE_DUPLICATE', N'{"source":"haya"}');
...
VALUES
('APP-0001', @benAhmed, @orgFood, @brFood, @supFood, 3000, N'APPROVED', N'HIGH', DATEADD(day,-15,SYSUTCDATETIME()), DATEADD(day,-14,SYSUTCDATETIME()), N'Ø·Ù„Ø¨ Ø¯Ø¹Ù… ØºØ°Ø§Ø¦ÙŠ', N'Ø­Ø³Ø¨ Ø§Ù„Ù…Ø­Ø§ÙØ¸Ø© ÙˆÙ†ÙˆØ¹ Ø§Ù„Ø¯Ø¹Ù…'),
('APP-0002', @benAhmed, @orgResala, @brResala, @supFood, 2500, N'APPROVED', N'HIGH', DATEADD(day,-13,SYSUTCDATETIME()), DATEADD(day,-12,SYSUTCDATETIME()), N'Ù†ÙØ³ Ù†ÙˆØ¹ Ø§Ù„Ø¯Ø¹Ù… Ù…Ù† Ø¬Ù…Ø¹ÙŠØ© Ø£Ø®Ø±Ù‰', N'Cross organization demo'),
('APP-0003', @benSara, @orgResala, @brResala, @supMed, 5000, N'UNDER_REVIEW', N'MEDIUM', DATEADD(day,-11,SYSUTCDATETIME()), NULL, N'Ø·Ù„Ø¨ Ø¯Ø¹Ù… Ø·Ø¨ÙŠ', N'Ù…Ø±Ø§Ø¬Ø¹Ø© Ù…Ø³ØªÙ†Ø¯Ø§Øª'),
('APP-0004', @benMona, @orgHaya, @brHaya, @supCloth, 1500, N'APPROVED', N'LOW', DATEADD(day,-9,SYSUTCDATETIME()), DATEADD(day,-8,SYSUTCDATETIME()), N'Ø¯Ø¹Ù… Ø¨Ø·Ø§Ø·ÙŠÙ†', N'Ø£Ù‚Ø±Ø¨ ÙØ±Ø¹'),
('APP-0005', @benYoussef, @orgFood, @brFood, @supFood, 1800, N'REJECTED', N'MEDIUM', DATEADD(day,-7,SYSUTCDATETIME()), DATEADD(day,-6,SYSUTCDATETIME()), N'Ù…Ø±ÙÙˆØ¶ Ù„Ù†Ù‚Øµ Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª', N'Ø¨ÙŠØ§Ù†Ø§Øª ØºÙŠØ± Ù…ÙƒØªÙ…Ù„Ø©');
...
VALUES
('CASE-0001', @app1, @benAhmed, @orgFood, @brFood, @supFood, N'Ø­Ø§Ù„Ø© Ø¯Ø¹Ù… ØºØ°Ø§Ø¦ÙŠ Ù„Ø£Ø­Ù…Ø¯ Ù…Ø­Ù…Ø¯', N'Ø­Ø§Ù„Ø© Ù…ÙƒØªÙ…Ù„Ø©', 3000, 3000, N'CLOSED', N'HIGH', DATEADD(day,-14,SYSUTCDATETIME()), DATEADD(day,-5,SYSUTCDATETIME())),
('CASE-0002', @app2, @benAhmed, @orgResala, @brResala, @supFood, N'Ø­Ø§Ù„Ø© Ø¯Ø¹Ù… ØºØ°Ø§Ø¦ÙŠ Ø«Ø§Ù†ÙŠØ© Ù„Ù†ÙØ³ Ø§Ù„Ù…Ø³ØªÙÙŠØ¯', N'ØªÙˆØ¶Ø­ Ø§Ù„ØªØ¹Ø§Ù…Ù„ Ù…Ø¹ Ø£ÙƒØ«Ø± Ù…Ù† Ø¬Ù…Ø¹ÙŠØ©', 2500, 1200, N'OPEN', N'HIGH', DATEADD(day,-12,SYSUTCDATETIME()), NULL),
('CASE-0003', @app3, @benSara, @orgResala, @brResala, @supMed, N'Ø­Ø§Ù„Ø© Ø¹Ù„Ø§Ø¬ Ù„Ø³Ø§Ø±Ø© Ù…Ø­Ù…ÙˆØ¯', N'Ø­Ø§Ù„Ø© Ù…ÙØªÙˆØ­Ø© Ù„Ø¯Ø¹Ù… Ø·Ø¨ÙŠ', 5000, 500, N'OPEN', N'MEDIUM', DATEADD(day,-10,SYSUTCDATETIME()), NULL),
('CASE-0004', @app4, @benMona, @orgHaya, @brHaya, @supCloth, N'Ø­Ø§Ù„Ø© Ø¨Ø·Ø§Ø·ÙŠÙ† ÙˆÙ…Ù„Ø§Ø¨Ø³ Ù„Ù…Ù†Ù‰', N'Ø­Ø§Ù„Ø© Ù…ÙƒØªÙ…Ù„Ø©', 1500, 1500, N'CLOSED', N'LOW', DATEADD(day,-8,SYSUTCDATETIME()), DATEADD(day,-2,SYSUTCDATETIME()));
...
VALUES
('DON-0001', N'Ù…ØªØ¨Ø±Ø¹ ØªØ¬Ø±ÙŠØ¨ÙŠ 1', '01077000001', 'donor1@test.com', @orgFood, @case1, @pmCash, 1500, N'CASE', N'Ø¯Ø¹Ù… ØºØ°Ø§Ø¦ÙŠ', 'DON-SEED-001', DATEADD(day,-13,SYSUTCDATETIME())),
('DON-0002', N'Ù…ØªØ¨Ø±Ø¹ ØªØ¬Ø±ÙŠØ¨ÙŠ 2', '01077000002', 'donor2@test.com', @orgFood, @case1, @pmWallet, 1500, N'CASE', N'Ø¯Ø¹Ù… ØºØ°Ø§Ø¦ÙŠ', 'DON-SEED-002', DATEADD(day,-12,SYSUTCDATETIME())),
('DON-0003', N'Ù…ØªØ¨Ø±Ø¹ ØªØ¬Ø±ÙŠØ¨ÙŠ 3', '01077000003', 'donor3@test.com', @orgResala, @case2, @pmCash, 1200, N'CASE', N'Ø¯Ø¹Ù… ØºØ°Ø§Ø¦ÙŠ', 'DON-SEED-003', DATEADD(day,-9,SYSUTCDATETIME())),
('DON-0004', N'Ù…ØªØ¨Ø±Ø¹ Ø¹Ø§Ù…', '01077000004', 'donor4@test.com', @orgHaya, NULL, @pmWallet, 800, N'ORGANIZATION_GENERAL', N'ØªØ¨Ø±Ø¹ Ø¹Ø§Ù…', 'DON-SEED-004', DATEADD(day,-6,SYSUTCDATETIME()));
...
VALUES
('INV-0001', @orgFood, @brFood, @itemFood, N'IN', 20, 450, NULL, NULL, N'MANUAL_ADJUSTMENT', N'SEED-IN-001', N'ØªÙˆØ±ÙŠØ¯ ØªØ¬Ø±ÙŠØ¨ÙŠ', DATEADD(day,-16,SYSUTCDATETIME())),
('INV-0002', @orgFood, @brFood, @itemFood, N'OUT', 3, 450, @case1, @app1, N'CASE', N'CASE-0001', N'ØµØ±Ù Ø¯Ø¹Ù… ØºØ°Ø§Ø¦ÙŠ Ù„Ø­Ø§Ù„Ø© Ø£Ø­Ù…Ø¯', DATEADD(day,-12,SYSUTCDATETIME())),
('INV-0003', @orgResala, @brResala, @itemFood, N'IN', 12, 450, NULL, NULL, N'MANUAL_ADJUSTMENT', N'SEED-IN-002', N'ØªÙˆØ±ÙŠØ¯ ØªØ¬Ø±ÙŠØ¨ÙŠ', DATEADD(day,-14,SYSUTCDATETIME())),
('INV-0004', @orgResala, @brResala, @itemFood, N'OUT', 2, 450, @case2, @app2, N'CASE', N'CASE-0002', N'ØµØ±Ù Ø¯Ø¹Ù… ØºØ°Ø§Ø¦ÙŠ Ù„Ù†ÙØ³ Ø§Ù„Ù…Ø³ØªÙÙŠØ¯ Ù…Ù† Ø¬Ù…Ø¹ÙŠØ© Ø£Ø®Ø±Ù‰', DATEADD(day,-9,SYSUTCDATETIME())),
('INV-0005', @orgHaya, @brHaya, @itemBlanket, N'IN', 10, 300, NULL, NULL, N'MANUAL_ADJUSTMENT', N'SEED-IN-003', N'ØªÙˆØ±ÙŠØ¯ Ø¨Ø·Ø§Ø·ÙŠÙ†', DATEADD(day,-11,SYSUTCDATETIME())),
('INV-0006', @orgHaya, @brHaya, @itemBlanket, N'OUT', 2, 300, @case4, @app4, N'CASE', N'CASE-0004', N'ØµØ±Ù Ø¨Ø·Ø§Ø·ÙŠÙ† Ù„Ù…Ù†Ù‰', DATEADD(day,-6,SYSUTCDATETIME()));
...
VALUES
(N'SAME_NATIONAL_ID_DIFFERENT_SOURCE', @benAhmed, @identityDupAhmed, @orgResala, '2990101123456', '01099000001', N'Ù†ÙØ³ Ø§Ù„Ø±Ù‚Ù… Ø§Ù„Ù‚ÙˆÙ…ÙŠ Ø¸Ù‡Ø± Ù…Ù† Ù…ØµØ¯Ø±/Ø¬Ù…Ø¹ÙŠØ© Ø£Ø®Ø±Ù‰ Ù„Ù†ÙØ³ Ø§Ù„Ù…Ø³ØªÙÙŠØ¯', 95, N'OPEN'),
(N'SUPPORT_OVERLAP_SAME_MONTH', @benAhmed, NULL, @orgResala, '2990101123456', '01099000001', N'Ø§Ù„Ù…Ø³ØªÙÙŠØ¯ Ø·Ù„Ø¨ Ù†ÙØ³ Ù†ÙˆØ¹ Ø§Ù„Ø¯Ø¹Ù… Ø§Ù„ØºØ°Ø§Ø¦ÙŠ Ù…Ù† Ø£ÙƒØ«Ø± Ù…Ù† Ø¬Ù…Ø¹ÙŠØ© ÙÙŠ Ù†ÙØ³ Ø§Ù„Ø´Ù‡Ø±', 92, N'OPEN');
...
VALUES
('FRD-0001', N'CROSS_ORGANIZATION_SUPPORT_OVERLAP', N'HIGH', 92, @benAhmed, @identityDupAhmed, @app2, @case2, @doc2, @orgResala, N'Ø§Ù„Ù…Ø³ØªÙÙŠØ¯ Ø£Ø­Ù…Ø¯ Ù…Ø­Ù…Ø¯ Ø¹Ù„ÙŠ Ø·Ù„Ø¨ Ù†ÙØ³ Ù†ÙˆØ¹ Ø§Ù„Ø¯Ø¹Ù… Ø§Ù„ØºØ°Ø§Ø¦ÙŠ Ù…Ù† Ø£ÙƒØ«Ø± Ù…Ù† Ø¬Ù…Ø¹ÙŠØ© Ø®Ù„Ø§Ù„ Ù†ÙØ³ Ø§Ù„Ø´Ù‡Ø±.', DATEADD(day,-5,SYSUTCDATETIME())),
('FRD-0002', N'DOCUMENT_REUSE_REVIEW', N'MEDIUM', 78, @benAhmed, NULL, @app2, @case2, @doc2, @orgFood, N'Ù…Ø³ØªÙ†Ø¯ Ù‡ÙˆÙŠØ© Ù…Ø³ØªØ®Ø¯Ù… ÙÙŠ Ø£ÙƒØ«Ø± Ù…Ù† Ø·Ù„Ø¨ Ù…Ø±ØªØ¨Ø· Ø¨Ø¬Ù…Ø¹ÙŠØ§Øª Ù…Ø®ØªÙ„ÙØ©. ÙŠØ­ØªØ§Ø¬ Ù…Ø±Ø§Ø¬Ø¹Ø© ÙˆÙ„ÙŠØ³ Ø±ÙØ¶ ØªÙ„Ù‚Ø§Ø¦ÙŠ.', DATEADD(day,-4,SYSUTCDATETIME()));
...
PRINT N'âœ… Reference and demo seed data inserted successfully.';
GO
...
/* ============================================================
   CLEAN DATABASE V1 â€” Views for Dashboards + Beneficiary 360
   ============================================================ */
...
SELECT
    ...
    STRING_AGG(CAST(o.organization_name_ar AS NVARCHAR(MAX)), N'ØŒ ') AS organizations_names,
...
PRINT N'âœ… Views created successfully.';
GO
...
/* ============================================================
   CLEAN DATABASE V2 â€” Advanced Engineering Layer
   Adds:
...
BEGIN
    RETURN CASE UPPER(ISNULL(@status, N''))
        WHEN N'SUBMITTED' THEN N'Ù…Ù‚Ø¯Ù…'
        WHEN N'UNDER_REVIEW' THEN N'Ù‚ÙŠØ¯ Ø§Ù„Ù…Ø±Ø§Ø¬Ø¹Ø©'
        WHEN N'APPROVED' THEN N'Ù…Ù‚Ø¨ÙˆÙ„'
        WHEN N'REJECTED' THEN N'Ù…Ø±ÙÙˆØ¶'
        WHEN N'OPEN' THEN N'Ù…ÙØªÙˆØ­Ø©'
        WHEN N'CLOSED' THEN N'Ù…ÙƒØªÙ…Ù„Ø©'
        WHEN N'COMPLETED' THEN N'Ù…ÙƒØªÙ…Ù„Ø©'
        WHEN N'PENDING_REVIEW' THEN N'Ù‚ÙŠØ¯ Ù…Ø±Ø§Ø¬Ø¹Ø© Ø§Ù„Ù…Ø³ØªÙ†Ø¯Ø§Øª'
        WHEN N'OPEN' THEN N'Ù…ÙØªÙˆØ­'
        ELSE ISNULL(@status, N'ØºÙŠØ± Ù…Ø­Ø¯Ø¯')
    END;
END;
GO
...
SELECT
    ...
    N'ØªÙ‚Ø¯ÙŠÙ… Ø·Ù„Ø¨' AS event_type_ar,
    a.application_code AS reference_code,
    o.organization_name_ar,
    st.support_name_ar,
    a.requested_amount AS amount,
    CAST(NULL AS DECIMAL(18,2)) AS quantity,
    CONCAT(N'ØªÙ… ØªÙ‚Ø¯ÙŠÙ… Ø·Ù„Ø¨ ', st.support_name_ar, N' Ø¥Ù„Ù‰ ', o.organization_name_ar) AS description_ar
...
SELECT
    ...
    CONCAT(N'ØªÙ… Ø¥Ù†Ø´Ø§Ø¡ Ø­Ø§Ù„Ø©: ', cc.case_title)
...
SELECT
    ...
    d.amount,
    CAST(NULL AS DECIMAL(18,2)),
    CONCAT(N'ØªÙ… Ø§Ø³ØªÙ‚Ø¨Ø§Ù„ ØªØ¨Ø±Ø¹ Ø¨Ù‚ÙŠÙ…Ø© ', FORMAT(d.amount, 'N0'), N' Ø¬Ù†ÙŠÙ‡ Ù„Ù„Ø­Ø§Ù„Ø© ', cc.case_code)
...
SELECT
    ...
    it.quantity,
    CONCAT(N'ØªÙ… ØµØ±Ù ', FORMAT(it.quantity, 'N0'), N' ', ii.unit, N' Ù…Ù† ', ii.item_name_ar)
...
SELECT
    ...
    CASE
        WHEN fa.risk_score >= 90 THEN N'Ø­Ø±Ø¬'
        WHEN fa.risk_score >= 75 THEN N'Ø¹Ø§Ù„ÙŠ'
        WHEN fa.risk_score >= 50 THEN N'Ù…ØªÙˆØ³Ø·'
        ELSE N'Ù…Ù†Ø®ÙØ¶'
...
SELECT
    ...
    s.average_unit_cost,
    CASE
        WHEN s.quantity_on_hand = 0 THEN N'Ù†ÙØ° Ø§Ù„Ù…Ø®Ø²ÙˆÙ†'
        WHEN s.quantity_on_hand <= 5 THEN N'Ù…Ø®Ø²ÙˆÙ† Ù…Ù†Ø®ÙØ¶'
        ELSE N'Ù…Ø³ØªÙ‚Ø±'
...
BEGIN
    ...
    SELECT
        ...
        ir.national_id,
        MIN(ir.phone),
        N'Ù†ÙØ³ Ø§Ù„Ø±Ù‚Ù… Ø§Ù„Ù‚ÙˆÙ…ÙŠ Ø¸Ù‡Ø± ÙÙŠ Ø£ÙƒØ«Ø± Ù…Ù† Ù…ØµØ¯Ø± Ø£Ùˆ Ø¬Ù…Ø¹ÙŠØ©.',
        95,
        N'OPEN'
    ...
    SELECT
        ...
        b.national_id,
        b.phone,
        CONCAT(N'Ø§Ù„Ù…Ø³ØªÙÙŠØ¯ Ø·Ù„Ø¨ Ù†ÙØ³ Ù†ÙˆØ¹ Ø§Ù„Ø¯Ø¹Ù… Ù…Ù† Ø£ÙƒØ«Ø± Ù…Ù† Ø¬Ù…Ø¹ÙŠØ© ÙÙŠ Ù†ÙØ³ Ø§Ù„Ø´Ù‡Ø±: ', st.support_name_ar),
        90,
        N'OPEN'
    FROM dbo.beneficiary_applications a
    JOIN dbo.beneficiary_profiles b ON a.beneficiary_id = b.beneficiary_id
...
BEGIN
    ...
    SELECT
        ...
        beneficiary_code,
        beneficiary_id,
        N'Ø§Ù„Ù…Ø³ØªÙÙŠØ¯ Ù„Ø§ ÙŠØ­ØªÙˆÙŠ Ø¹Ù„Ù‰ Ø±Ù‚Ù… Ù‡Ø§ØªÙ.'
    ...
    SELECT
        ...
        beneficiary_id,
        organization_id,
        N'Ø§Ù„Ø­Ø§Ù„Ø© Ù…ÙØªÙˆØ­Ø© Ø£Ùˆ Ù…Ù†Ø´ÙˆØ±Ø© Ù„ÙƒÙ† Ù‚ÙŠÙ…Ø© Ø§Ù„Ø¯Ø¹Ù… Ø§Ù„Ù…Ø·Ù„ÙˆØ¨Ø© ØºÙŠØ± ØµØ­ÙŠØ­Ø©.'
    FROM dbo.charity_cases
    ...
    SELECT
        ...
        donation_code,
        organization_id,
        N'Ù†ÙˆØ¹ Ø§Ù„ØªØ¨Ø±Ø¹ Ù„Ø§ ÙŠØªÙˆØ§ÙÙ‚ Ù…Ø¹ case_id / organization_id.'
    FROM dbo.donations
    WHERE
...
PRINT N'âœ… Advanced objects created successfully.';
GO
...
/* ============================================================
   CLEAN DATABASE V2 â€” Automation Triggers + Performance Indexes
   ============================================================ */
...
PRINT N'âœ… Automation triggers and indexes created successfully.';
GO
...
/* ============================================================
   V2.1 FINAL FIX â€” Beneficiary 360 Procedure
   Fixes:
...
-- 4) Quick validation
------------------------------------------------------------
PRINT N'âœ… V2.1 Beneficiary 360 procedure fixed successfully.';
...
/* ============================================================
   FINAL QUICK VALIDATION â€” NO ERROR VERSION
   ============================================================ */

PRINT N'âœ… All-in-one NO ERRORS database script completed.';
...
PRINT N'âœ… Beneficiary 360 procedure smoke test:';
...
BEGIN
    ...
    IF UPPER(ISNULL(@emergency_level, '')) IN ('HIGH', 'CRITICAL', N'Ø¹Ø§Ù„ÙŠ', N'Ø­Ø±Ø¬') SET @score += 15;
    IF UPPER(ISNULL(@emergency_level, '')) IN ('MEDIUM', N'Ù…ØªÙˆØ³Ø·') SET @score += 7;
...
SELECT
    ...
    c.case_title,
    c.case_description,
    COALESCE(c.public_display_name, CONCAT(N'Ù…Ø³ØªÙÙŠØ¯ Ø±Ù‚Ù… ', bp.beneficiary_id)) AS public_display_name,
    g.governorate_name_ar,
    c.required_amount,
    c.collected_amount,
    ...
    CASE
        WHEN c.case_status IN (N'CLOSED', N'FUNDED') OR c.collected_amount >= c.required_amount
            THEN N'ØºÙŠØ± Ù…Ø³ØªØ­Ù‚ Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±'
...
WITH current_month AS (
    ...
    SELECT
        ...
        STRING_AGG(CONCAT(o.organization_name_ar, N': ', st.support_name_ar, N' - ', FORMAT(sd.amount_value, 'N0'), N' Ø¬Ù†ÙŠÙ‡'), N' | ') AS support_summary_ar
...
SELECT
    ...
    COALESCE(ms.support_summary_ar, N'Ù„Ù… ÙŠØ­ØµÙ„ Ø¹Ù„Ù‰ Ø¯Ø¹Ù… Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±') AS support_summary_ar,
    ...
    CASE
        WHEN COALESCE(fr.max_fraud_level_num, 0) >= 3 THEN N'Ù…Ø±Ø§Ø¬Ø¹Ø© ÙŠØ¯ÙˆÙŠØ© Ø¨Ø³Ø¨Ø¨ ØªÙ†Ø¨ÙŠÙ‡Ø§Øª Ø§Ø­ØªÙŠØ§Ù„'
        WHEN COALESCE(ms.support_count_this_month, 0) > 0 THEN N'ØºÙŠØ± Ù…Ø³ØªØ­Ù‚ Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±'
        ELSE N'Ù…Ø³ØªØ­Ù‚ Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±'
...
USING (VALUES
    (N'NATIONAL_ID', N'ØµÙˆØ±Ø© Ø§Ù„Ø¨Ø·Ø§Ù‚Ø©', N'National ID', 1),
    (N'INCOME_PROOF', N'Ø¥Ø«Ø¨Ø§Øª Ø¯Ø®Ù„', N'Income Proof', 1),
    (N'MEDICAL_REPORT', N'ØªÙ‚Ø±ÙŠØ± Ø·Ø¨ÙŠ', N'Medical Report', 0),
    (N'RENT_RECEIPT', N'Ø¥ÙŠØµØ§Ù„ Ø¥ÙŠØ¬Ø§Ø±', N'Rent Receipt', 0),
    (N'CHILD_BIRTH_CERT', N'Ø´Ù‡Ø§Ø¯Ø© Ù…ÙŠÙ„Ø§Ø¯ Ø§Ù„Ø£Ø·ÙØ§Ù„', N'Children Birth Certificate', 0)
...
BEGIN
    INSERT INTO dbo.platform_users (user_code, role_id, organization_id, branch_id, full_name, phone, email, password_hash)
    VALUES ('USR-HAYA-ADMIN', @roleCharity, @orgHaya, @brHaya, N'Ø£Ø¯Ù…Ù† Ø­ÙŠØ§Ø© ÙƒØ±ÙŠÙ…Ø©', '01030000004', 'haya.admin@test.com', 'demo_hash');
END;

IF @roleDonor IS NOT NULL AND NOT EXISTS (SELECT 1 FROM dbo.platform_users WHERE email = 'donor@test.com')
BEGIN
    INSERT INTO dbo.platform_users (user_code, role_id, organization_id, branch_id, full_name, phone, email, password_hash)
    VALUES ('USR-DONOR-001', @roleDonor, NULL, NULL, N'Ù…ØªØ¨Ø±Ø¹ ØªØ¬Ø±ÙŠØ¨ÙŠ', '01077000001', 'donor@test.com', 'demo_hash');
END;
GO
...
BEGIN
    ...
    BEGIN
        ...
        SET @reason = N'ÙŠÙˆØ¬Ø¯ ØªÙ†Ø¨ÙŠÙ‡ Ø§Ø­ØªÙŠØ§Ù„ Ø¹Ø§Ù„ÙŠ/Ø­Ø±Ø¬ ÙˆÙŠØ­ØªØ§Ø¬ Ù…Ø±Ø§Ø¬Ø¹Ø© ÙŠØ¯ÙˆÙŠØ©.';
    END
    ELSE IF @support_count > 0
    BEGIN
        SET @status = N'NOT_ELIGIBLE_THIS_MONTH';
        SET @reason = N'Ø­ØµÙ„ Ø¹Ù„Ù‰ Ø¯Ø¹Ù… Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±.';
    END
    ELSE
    BEGIN
        SET @status = N'ELIGIBLE';
        SET @reason = N'Ù„Ù… ÙŠØ­ØµÙ„ Ø¹Ù„Ù‰ Ø¯Ø¹Ù… Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±.';
    END
...
BEGIN
    ...
    BEGIN
        ...
        VALUES (@support_code, @beneficiary_id, @organization_id, @branch_id, @application_id, @case_id, @support_type_id, @month, N'DONATION', @collected, N'Ø¯Ø¹Ù… Ù…Ø§Ù„ÙŠ Ù…ÙƒØªÙ…Ù„ Ù…Ù† Ø§Ù„Ù…ØªØ¨Ø±Ø¹ÙŠÙ†', N'ØªÙ… ØªØ³Ø¬ÙŠÙ„Ù‡ ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹ Ø¹Ù†Ø¯ Ø§ÙƒØªÙ…Ø§Ù„ Ù…Ø¨Ù„Øº Ø§Ù„Ø­Ø§Ù„Ø©.');
    END
...
SELECT
    ...
    c.documents_verified,
    CASE
        WHEN c.case_status IN (N'CLOSED', N'FUNDED') OR c.collected_amount >= c.required_amount THEN N'ØºÙŠØ± Ù…Ø³ØªØ­Ù‚ Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±'
        WHEN c.eligibility_status <> N'ELIGIBLE' THEN N'ØºÙŠØ± Ù…Ø³ØªØ­Ù‚ Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±'
        ELSE N'Ù…Ø³ØªØ­Ù‚'
    END AS eligibility_label_ar,
    CASE
        WHEN c.donation_enabled = 1
...
WITH current_month AS (
    ...
    SELECT beneficiary_id, COUNT(*) AS active_applications
    FROM dbo.beneficiary_applications
    WHERE application_status IN (N'SUBMITTED', N'ASSIGNED', N'UNDER_REVIEW', N'APPROVED', N'Ù‚ÙŠØ¯ Ø§Ù„Ù…Ø±Ø§Ø¬Ø¹Ø©', N'Ù…Ù‚Ø¨ÙˆÙ„')
    GROUP BY beneficiary_id
), active_cases AS (
    SELECT beneficiary_id, COUNT(*) AS active_cases
    FROM dbo.charity_cases
    WHERE case_status IN (N'OPEN', N'PUBLISHED', N'Ù…ÙØªÙˆØ­Ø©', N'Ù…Ù†Ø´ÙˆØ±Ø©')
    GROUP BY beneficiary_id
), fraud AS (
    SELECT
        beneficiary_id,
    ...
    FROM dbo.fraud_alerts
    WHERE alert_status IN (N'OPEN', N'UNDER_REVIEW', N'Ù…ÙØªÙˆØ­', N'Ù‚ÙŠØ¯ Ø§Ù„Ù…Ø±Ø§Ø¬Ø¹Ø©')
    GROUP BY beneficiary_id
)
SELECT
    bp.beneficiary_id,
...
/* -----------------------------------------------------------------------------
   BACKEND SQL LOGIN
   The FastAPI backend uses this SQL login by default. This avoids Windows
   Authentication / Guest-user issues on local machines and Docker SQL Server.
----------------------------------------------------------------------------- */
USE master;
GO

IF SUSER_ID(N'charity_app_user') IS NULL
BEGIN
    CREATE LOGIN charity_app_user
    WITH PASSWORD = 'ChangeMe_StrongPassword_2026!', CHECK_POLICY = OFF, CHECK_EXPIRATION = OFF;
END;
ELSE
BEGIN
    ALTER LOGIN charity_app_user WITH PASSWORD = 'ChangeMe_StrongPassword_2026!', CHECK_POLICY = OFF, CHECK_EXPIRATION = OFF;
END;
GO

ALTER LOGIN charity_app_user ENABLE;
GO

USE unified_charity_platform_clean;
GO
IF USER_ID(N'charity_app_user') IS NULL CREATE USER charity_app_user FOR LOGIN charity_app_user;
GO
IF ISNULL(IS_ROLEMEMBER(N'db_owner', N'charity_app_user'), 0) <> 1 ALTER ROLE db_owner ADD MEMBER charity_app_user;
GO
...
USE master;
GO
PRINT N'Backend SQL login charity_app_user is ready. Use this in backend/.env:';
PRINT N'SQL_SERVER_USER=charity_app_user';
PRINT N'SQL_SERVER_PASSWORD=CharityApp@2026';
GO
...
PRINT N'FULL PHASE 3 SETUP FINISHED SUCCESSFULLY. You can now start backend/frontend, then start the Data Engineering phase.';
GO
USE unified_charity_platform_clean;
GO
